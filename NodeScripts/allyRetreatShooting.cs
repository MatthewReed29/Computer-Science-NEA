...
public class allyRetreatShooting : templateNode
{
    //if at optimal range where team is near but not very near, this function has high weighting
    // when there are more teammates
    // when the teams are separated
    public override float poll_priority(awareness known, relevant_information rel, decisions decision_script)
    {
        ...
        else
        {
            ...
            float y = ((-x) * (x - (1.5f * (decision_script.team_stay_range_natural)))) / Mathf.Pow(0.75f * decision_script.team_stay_range_natural, 2);
            if (y < 0)
            {
                y = 0f;
            }
